...
#execute as @e[type=!#timelesscraft:not_mob,tag=!raycasting,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function timelesscraft:raycasttimeless/timeraycastcollide

## .distance can be any fake player name
#scoreboard players add .distance timeless_rcast 1
...
#execute if score .distance timeless_rcast matches ..1000 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #timelesscraft:raycast_pass run function timelesscraft:raycasttimeless/timeraycast
