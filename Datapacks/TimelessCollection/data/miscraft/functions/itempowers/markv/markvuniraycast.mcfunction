...
execute as @e[type=!#timelesscraft:not_mob,tag=!mkvuraycasting,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function miscraft:itempowers/markv/markvuniraycastcollide

# .distance can be any fake player name
scoreboard players add .distance mkvurcast 1
...
execute if score .distance mkvurcast matches ..1000 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #timelesscraft:raycast_pass run function miscraft:itempowers/markv/markvuniraycast
