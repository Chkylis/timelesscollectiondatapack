...
execute as @e[type=!#timelesscraft:not_mob,tag=!aosmmraycasting,dx=0] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] positioned ~0.99 ~0.99 ~0.99 run function destiny2:exotic/mementomoriraycast/mementomoriraycastcollide

## .distance can be any fake player name
scoreboard players add .distance mentomorircast 1
...
execute if score .distance mentomorircast matches ..450 positioned ^ ^ ^0.1 rotated ~ ~ if block ~ ~ ~ #timelesscraft:raycast_pass run function destiny2:exotic/mementomoriraycast/mementomoriraycast
